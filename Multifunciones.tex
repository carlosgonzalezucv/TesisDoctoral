...
\section{Transformación de Takagi-Tabor de una multifunción.}

Motivados por el t\'ermino de error introducido en el \thm{TabTab3}
por los profesores Ja. Tabor y J\'o. Tabor, se introduce la siguiente
definici\'on

\Defi{TakTabTransf}{
Sean $D$ un conjunto estrellado con respecto al origen, 
y $S:D\to\P_0(Y)$ una multifunci\'on con la propiedad de 
que $0\in S(x)$ para todo $x\in D$.
Para dicha multifunción, se define la transformaci\'on
de Takagi-Tabor $S^\perp:\R\times D\to Y$ de la siguiente manera
\Eq{TakTabTransf}{
 S^\perp(t,x):=\cl\bigg(\bigcup_{n=0}^{\infty} \sum_{k=0}^{n} 
                 2d_{\Z}(2^kt)S\bigg(\frac{x}{2^k}\bigg)\bigg)\qquad(t\in\R,\,x\in D).
}
}

alkjdslajdslkjadjaldjs
alkjsdalkjsdlakjd
lakjdslkajdslkjads
lkajdlakjdslakjds
lakjdslakjdsalkjd